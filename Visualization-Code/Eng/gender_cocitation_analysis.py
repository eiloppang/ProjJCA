...
def create_co_citation_matrix(references_df, target_authors, output_path):
    ...
    for art_id, authors in grouped.items():
        # Remove duplicates within the same paper if necessary? 
        # The original script didn't unique() inside the loop but grouped.unique() was commented out.
        # However, standard co-citation usually counts unique co-occurrences per paper.
        # If an author is cited twice in the same paper, does it count? 
        # Usually co-citation is binary per paper: did Author A and Author B appear together?
        # Let's enforce unique authors per paper to avoid self-loops or double counting same-pair in same paper
...
def main():
    ...
    pkl_path = '/Users/cosi/Library/CloudStorage/GoogleDrive-vadoropupille@gmail.com/공유 드라이브/ProjJCA/[최종] 작업 데이터/영어영문.pkl'
    
    references_file = os.path.join(base_dir, "영문학_참고문헌_분석용.csv")
    ...
    try:
        ...
        # Load Metadata (PKL)
        # Handle potential pandas/numpy version issues by try/except block just in case, 
        # though environment should be fixed now.
        try:
    ...
    # Merge
    # We want to keep all references, but attach gender info from the paper metadata
    # Left join on ref_df
